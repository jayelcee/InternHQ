/**
 * Service for handling internship completion requests and document generation.
 * Consolidates all completion-related logic for maintainability.
 */
...
export class CompletionService {
  /**
   * Gets a completion request with all related data by ID.
   */
  ...
  /**
   * Gets all completion requests with optional status filter and pagination.
   */
  static async getAllCompletionRequests(
    ...
    try {
      ...
      if (!tablesExist) await initializeDatabase()
  ...
  /**
   * Creates a new completion request for a user and internship program.
   */
  ...
  /**
   * Processes a completion request (approve or reject).
   */
  ...
  /**
   * Generates an official DTR document for a completion request.
   */
  static async generateDTRDocument(
    ...
    try {
      ...
      const timeLogsDetails = timeLogsResult.map(log => ({
        ...
        timeOut: log.time_out ? new Date(log.time_out).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true }) : '',
  ...
  /**
   * Generates a certificate document for a completion request.
   */
  ...
  /**
   * Gets a user's latest completion request and associated documents.
   */
  ...
  /**
   * Deletes a completion request and associated documents (admin only).
   */
